...
func InitK8sClients() (*kubernetes.Clientset, *apiextensionsclient.Clientset, error) {
	config, err := getConfig()
	if err != nil {
		return nil, nil, fmt.Errorf("error getting config for k8s client: %v", err)
	}
	...
	clientsetextension, err := apiextensionsclient.NewForConfig(config)
	if err != nil {
		return nil, nil, fmt.Errorf("error creating clientset extension: %v", err)
	}

	return clientset, clientsetextension, nil
}
