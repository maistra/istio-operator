...
func (r *controlPlaneInstanceReconciler) processComponentManifests(ctx context.Context, chartName string) (madeChanges bool, err error) {
	...
	mp := helm.NewManifestProcessor(r.ControllerResources, helm.NewPatchFactory(r.Client), r.Instance.GetNamespace(), r.meshGeneration, r.Instance, r.preprocessObject, r.processNewObject, r.preprocessObjectForPatch)
