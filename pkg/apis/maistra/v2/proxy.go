...
type ProxyConfig struct {
	...
	// `AdminPort` configures the admin port exposed by the sidecar. Defaults to 15000
	// Maps to `defaultConfig.proxyAdminPort`.
	// +optional
	AdminPort int32 `json:"adminPort,omitempty"`
	// Concurrency configures the number of threads that should be run by the sidecar. Defaults to 2.
...
type ProxyNetworkingConfig struct {
	...
	// `ConnectionTimeout` configures the ammount of time before a connection times out. Defaults to 10s.
	// Maps to `meshConfig.defaultConfig.connectionTimeout`
	// +optional
...
type ProxyNetworkInitConfig struct {
	...
	// the pod's networking. If `InitContainer` is used, `istio_cni.enabled` will be set to `false`.
	// +optional
...
type ProxyTrafficControlConfig struct {
	// `Inbound` configures what inbound traffic is routed through the sidecar. Defaults to * (all ports).
...
type ProxyInboundTrafficControlConfig struct {
	...
	// `IncludedPorts` to be routed through the sidecar. Set to `*` (all ports), or a comma separated list of integers. Defaults to `*`.
	// `.Values.global.proxy.includeInboundPorts`, overridden by `traffic.sidecar.istio.io/includeInboundPorts`
	// +optional
	IncludedPorts []string `json:"includedPorts,omitempty"`
	// `ExcludedPorts` configures ports to be routed around the sidecar. Defaults to an empty list.
	// `.Values.global.proxy.excludeInboundPorts`, overridden by `traffic.sidecar.istio.io/excludeInboundPorts`
	// +optional
	ExcludedPorts []int32 `json:"excludedPorts,omitempty"`
}

// `ProxyOutboundTrafficControlConfig` configures what outbound traffic is routed
// through the sidecar.
type ProxyOutboundTrafficControlConfig struct {
	...
	// * or comma separated list of CIDR (classless internet domain routing) addresses.
	// +optional
	IncludedIPRanges []string `json:"includedIPRanges,omitempty"`
	// `ExcludedIPRanges` specifies which outbound IP ranges should _not_ be routed through the sidecar. Accepted values are either `*` or a comma separated list of CIDR (classless internet domain routing) addresses.
...
type ProxyReadinessConfig struct {
	...
	// rewrite application container probes to be routed through the sidecar. Defaults to false
	// `.Values.sidecarInjectorWebhook.rewriteAppHTTPProbe`
	// +optional
	RewriteApplicationProbes bool `json:"rewriteApplicationProbes,omitempty"`
	// `StatusPort` specifies the default port number for Pilot agent health checks. A value of `0` will disable health checking. Defaults to 15020.
	// `.Values.global.proxy.statusPort`, overridden by `status.sidecar.istio.io/port`
	// +optional
	StatusPort int32 `json:"statusPort,omitempty"`
	// `InitialDelaySeconds` specifies the initial delay for the readiness probe. Defaults to 1.
	...
	// `PeriodSeconds` specifies the period over which the probe is checked.Defaults to 2.
	// `.Values.global.proxy.readinessPeriodSeconds`, overridden by `readiness.status.sidecar.istio.io/periodSeconds`
	// +optional
	PeriodSeconds int32 `json:"periodSeconds,omitempty"`
	// `FailureThreshold` represents the number of consecutive failures before the container is marked as not ready. Defaults to 30.
...
type ProxyFileAccessLogConfig struct {
	...
	// `Encoding` to use when writing access log entries.  Either `JSON` or `TEXT`
	// may be specified.
