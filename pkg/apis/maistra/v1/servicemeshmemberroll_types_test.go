...
func TestMatch(t *testing.T) {
	tests := []struct {
		...
		{name: "match-all", pattern: "*", expect: true},
		{name: "exact-match", pattern: "exact-match", expect: true},
		{name: "no-match", pattern: "bar", expect: false},
		{name: "prefix-match", pattern: "prefix*", expect: true},
		{name: "prefix-no-match", pattern: "bar*", expect: false},
		{name: "empty-pattern", pattern: "", expect: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := match(tt.name, tt.pattern); got != tt.expect {
				t.Errorf("match(%q, %q) = %v, expected %v", tt.name, tt.pattern, got, tt.expect)
			}
		})
	}
}
