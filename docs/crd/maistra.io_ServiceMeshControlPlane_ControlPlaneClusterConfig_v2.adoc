...
| meshExpansion
| `.Values.global.meshExpansion.enabled`, if not null i.e. does MultiCluster require mesh expansion ports to be configured on the ingress gateway?
| <<MeshExpansionConfig>>
...
`GatewayConfig` represents the configuration for a gateway the user be forced to add these manually?
...
| meshNetworks
| `.Values.global.meshNetworks`  ```  <spec.cluster.network>:      endpoints:      - fromRegistry: <spec.cluster.name>      gateways:      - service: <ingress-gateway-service-name>        port: 443 # mtls port  ```
