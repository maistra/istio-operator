...
| persistence
| `Persistence` configures a PersistentVolume associated with the grafana installation `.Values.grafana.persist`, true if not null
| <<ComponentPersistenceConfig>>

| security
| `Security` is used to secure the grafana service. `.Values.grafana.security.enabled`, true if not null
...
| env
| `Env` allows specification of various grafana environment variables to be configured on the Grafana container. .Values.grafana.env
| map[string]string

| envSecrets
| `EnvSecrets` allows specification of secret fields into grafana environment variables to be configured on the grafana container `.Values.grafana.envSecrets`
| map[string]string
...
`JaegerIngressConfig` configures Kubernetes Ingress or OpenShift Route for exposing Jaeger services.
...
| indexCleaner
| `IndexCleaner` represents the configuration for the elasticsearch index cleaner `.Values.tracing.jaeger.elasticsearch.esIndexCleaner`, raw yaml
| <<maistrav1_HelmValues>>

| nodeCount
| `NodeCount` represents the number of elasticsearch nodes to create. Defaults to 3. `.Values.tracing.jaeger.elasticsearch.nodeCount`
| integer
...
| storage
| Storage represents storage configuration for elasticsearch. `.Values.tracing.jaeger.elasticsearch.storage`, raw yaml
| <<maistrav1_HelmValues>>
...
| maxTraces
| `MaxTraces` configures the maximum number of traces to store. Defaults to 100000. `.Values.tracing.jaeger.memory.max_traces`
| integer
...
| service
| `Service` is used to configure the k8s Service associated with the Kiali installation. Ingress is used for enablement and context path. for enablement and contextPath
| <<ComponentServiceConfig>>
...
| address
| Address of existing prometheus installation implies .Values.kiali.prometheusAddr
| string
...
| `MetricsExpiryDuration` is the duration to hold metrics. (mixer/v1 only) Defaults to 10m `.Values.mixer.adapters.prometheus.metricsExpiryDuration`
| string
...
| retention
| `Retention` specifies how long metrics should be retained by Prometheus. Defaults to 6h. `.Values.prometheus.retention`
| string

| scrapeInterval
| `ScrapeInterval` specifies how frequently Prometheus should scrape pods for metrics. Defaults to 15s. `.Values.prometheus.scrapeInterval`
| string

| selfManaged
| `SelfManaged` specifies whether or not the Maistra operator or Prometheus operator should manage the Prometheus installation. Set `SelfManaged` to `true` to use the Maistra operator or `false` to use the Prometheus operator. No support is provided for the Prometheus operator. Governs use of either prometheus charts or prometheusOperator charts.
| boolean

| service
| `Service` allows for customization of the Kubernetes Service associated with the Prometheus installation.
| <<ComponentServiceConfig>>

| useTLS
| `UseTLS` configures whether the Prometheus server should use TLS. `.Values.prometheus.provisionPrometheusCert` 1.6+
| boolean
