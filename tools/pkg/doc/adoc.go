...
var asciidocTemplate *template.Template

func init() {
	asciidocTemplate = template.Must(template.New("AsciidocTemplate").Funcs(templateFuncMap).Parse(asciidocTemplateText))
}

const asciidocTemplateText = `
{{ define "CRDSummary" -}}
{{ $input := . -}}
= Custom Resources
...
link:{{ fileForKind $kind "adoc" }}[{{ $kind.Kind }}]:: {{ $schema.Description }}
...
Used by link:{{ fileForKind .Kind "adoc" }}[{{ .Kind.Kind }}({{ .Kind.Group }}/{{ .Kind.Version }})]
...
| {{ template "FieldType" dict "Field" $property "Kind" $input.Kind "Root" $input.Root "Locations" $input.Locations "External" $input.External }}
...
{{ if .Field.Ref -}}
    {{ if .External -}}
        ...
        {{ if $location -}}
link:{{ $location }}[{{ refName .Field.Ref }}]
        {{- else -}}
{{ refName .Field.Ref -}}
        {{ end -}}
    {{- else -}}
