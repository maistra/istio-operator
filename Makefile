...
MAISTRA_VERSION        ?= 2.2.10
MAISTRA_BRANCH         ?= maistra-2.2
REPLACES_PRODUCT_CSV   ?= 2.2.9
REPLACES_COMMUNITY_CSV ?= 2.2.9
VERSION                ?= development
...
collect-2.2-templates:
	...
	find ${RESOURCES_DIR}/smcp-templates/v2.2/ -maxdepth 1 -type f ! -name "maistra" ! -name "servicemesh" -exec rsync -av {} ${TEMPLATES_OUT_DIR}/v2.2/ \;
